...
import { checkExecutionAllowed } from "./execution_guard";
...
async function ensureDatabaseSchema(env: Env): Promise<void> {
  ...
  const columns = await env.DB
    .prepare(
      "SELECT name, sql FROM sqlite_master WHERE type='table' AND name IN ('tokens','market_snapshots','signals','trades')"
    )
    .all<{ name: string; sql: string }>();

  const sqlByTable = new Map(
    (columns.results ?? []).map(row => [row.name, row.sql || ""])
  );
...
async function writeRuntimeState(
  ...
  const holdingTelemetryOnly = Object.keys(patch).every(
    key =>
      key === "lastHoldingCheck" ||
      key === "paperUnrealizedPnlUsd"
  );

  if (holdingTelemetryOnly) {
    const lastPersisted = Number(
      current.lastHoldingCheck || 0
    );

    if (
      lastPersisted > 0 &&
      Date.now() - lastPersisted <
...
export default {
  async fetch(
    ...
    if (url.pathname === "/status") {
      if (
        url.searchParams.get("telegramTest") === "1"
      ) {
...
export class TradingEngine {
  ...
  async fetch(
    ...
    if (
      body.action === "paper-signal" &&
      Number.isInteger(body.signalId)
    ) {
      return json(
...
async function runHoldingCheck(
  env: Env
) {
  ...
  try {
    ...
    const state =
      await readRuntimeState(env);

    const monUsd = Number(
      state.monUsd ||
      await env.CIEL_STATE.get("mon_usd")
    );

    if (!(monUsd > 0)) {
      await writeRuntimeState(
        env,
        { lastHoldingCheck: checkedAt }
      );
      return;
    }
    ...
    if (
      ...
      await runPaperPositionMonitoring(
        env,
        monUsd
      );
    }
    ...
    const tokens =
      await env.DB
        .prepare(
          "SELECT address,decimals FROM tokens WHERE graduated=1"
        )
        .all<{
          address: string;
          decimals: number;
    ...
    for (
      const row of tokens.results || []
    ) {
      const balance =
        await tokenBalance(
          publicClient(env.NAD_RPC_URL),
    ...
    console.error(
      `Holding check failed: ${String(error).slice(0, 1000)}`
    );
  }
}
  ...
  try {
    ...
    await writeRuntimeState(
      env,
      {
        lastMarketCycle:
          cycleAt,
        lastMarketCycleError:
          undefined,
        lastIndexerSnapshots:
          result?.snapshots || 0,
        lastIndexerCreates:
          result?.creates || 0,
        lastIndexerBuys:
...
async function maybeWriteModelSignal(
  env: Env,
  current: Snapshot,
  analysis:
    Awaited<ReturnType<typeof askGemini>>
) {
  if (
    !analysis ||
    !["BUY", "SELL"].includes(
  ...
  const existingPosition =
    await env.DB
      .prepare(
        "SELECT quantity FROM positions WHERE token_address=? AND quantity<>'0'"
      )
      .bind(current.token)
      .first<{
        quantity: string;
...
async function runModelMaintenance(
  env: Env
) {
  const maintenanceAt =
    Date.now();
  ...
  try {
    ...
    if (!candidates.length) {
      await writeRuntimeState(
        env,
        {
          lastModelMaintenance:
            maintenanceAt,
          lastModelError:
            `No established high-volume meme candidates above $${PATTERN_MIN_MARKET_CAP_USD.toLocaleString()} yet`
        }
      );
    ...
    for (
      ...
      decisions.push(
        `${current.token}:${analysis.action}:${(analysis.confidence * 100).toFixed(0)}%:${analysis.regime}`
      );
    }
    ...
    if (analyzed > 0) {
      ...
      await writeRuntimeState(
        ...
        {
          lastGeminiError:
            `Established candidates above $${PATTERN_MIN_MARKET_CAP_USD.toLocaleString()} were found but none produced a valid Gemini decision`
        }
      );
    }
    ...
    console.error(
      `Model maintenance failed: ${message}`
    );
  }
}
...
async function status(
  ...
  try {
    if (indexerRaw) {
      indexerState =
        JSON.parse(indexerRaw);
    }
  } catch {}

  const telegramRaw =
    await env.CIEL_STATE.get(
      "ciel_telegram_runtime"
    );
  ...
  return {
    ...
    telegramLastAttemptAt:
      telegramState.lastAttemptAt ||
      null,

    telegramLastSuccessAt:
      telegramState.lastSuccessAt ||
      null,

    telegramLastFailureAt:
      telegramState.lastFailureAt ||
      null,
    ...
    telegramLastTestAt:
      telegramState.lastTestAt ||
      null,

    telegramLastTestSuccess:
      telegramState.lastTestSuccess ??
      null,
    ...
    lastHoldingCheck:
      state.lastHoldingCheck ||
      null,

    lastMarketCycle:
      state.lastMarketCycle ||
      indexerState.lastRunMs ||
      null,

    lastMarketCycleError:
      state.lastMarketCycleError ||
      null,

    lastModelMaintenance:
      state.lastModelMaintenance ||
      null,

    lastModelAnalyzed:
      state.lastModelAnalyzed ||
      null,
    ...
    lastIndexerAttempt:
      state.lastIndexerAttempt ||
      null,

    lastIndexerSnapshots:
      state.lastIndexerSnapshots ??
      null,

    lastIndexerCreates:
      state.lastIndexerCreates ??
      null,

    lastIndexerBuys:
      state.lastIndexerBuys ??
      null,

    lastIndexerSells:
      state.lastIndexerSells ??
      null,

    lastGeminiAttempt:
      state.lastGeminiAttempt ||
      null,

    lastGeminiSuccess:
      state.lastGeminiSuccess ||
      null,
...
async function getPaperBalance(
  ...
  if (raw === null) {
    await env.CIEL_STATE.put(
      PAPER_BALANCE_KEY,
      String(PAPER_INITIAL_BALANCE_MON)
    );
...
async function beginPaperExecution(
  env: Env,
  signal: {
    id: number;
    token_address: string;
  ...
  return {
    ...
    row:
      await env.DB
        .prepare(
          "SELECT * FROM paper_executions WHERE signal_id=?"
        )
        .bind(signal.id)
        .first<any>()
  };
...
async function failPaperExecution(
  env: Env,
  signalId: number,
  error: unknown
) {
...
async function executePaperSignal(
  env: Env,
  signalId: number
) {
  if (
  ...
  if (
    ...
    return {
      ok: false,
      skipped:
        "paper execution circuit breaker is open"
    };
  }
  ...
  if (
    ...
    return {
      ok: true,
      skipped:
        "paper execution already consumed",
      executionState:
        existing.state
    };
  }
    ...
    if (
      ...
      if (
        ...
        return consumeSignal(
          env,
          signalId,
          "paper skipped: insufficient balance or MON/USD price"
        );
      }
      ...
      if (
        ...
        return consumeSignal(
          env,
          signalId,
          "paper skipped: fresh buy quote returned zero"
        );
      }
      ...
      /*
       * Paper mode is intentionally
       * independent from live mode.
       *
       * This gate validates the trade
       * without disabling paper trading
       * merely because TRADING_ENABLED
       * is true.
       */
      ...
      if (
        ...
        return consumeSignal(
          env,
          signalId,
          `paper skipped: execution guard rejected BUY: ${executionCheck.reasons.join(", ")}`
        );
      }
      ...
      if (
        ...
        return consumeSignal(
          env,
          signalId,
          `paper skipped: risk evaluation rejected BUY: ${risk.reasons.join(", ")}`
        );
      }
    ...
    if (
      ...
      if (
        ...
        return consumeSignal(
          env,
          signalId,
          "paper skipped: no position to sell"
        );
      }
      ...
      if (
        ...
        return consumeSignal(
          env,
          signalId,
          "paper skipped: fresh sell quote returned zero"
        );
      }
      ...
      if (
        ...
        return consumeSignal(
          env,
          signalId,
          `paper skipped: execution guard rejected SELL: ${executionCheck.reasons.join(", ")}`
        );
      }
      ...
      await notifyTelegram(
        env,
        `📝 Ciel PAPER SELL\nToken: ${token}\nProceeds: ${proceedsMon} MON\nRealized PnL: $${realized.toFixed(4)}`
      );
