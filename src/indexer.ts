...
function rawToUnits(value: bigint, decimals: number): number {
...
export async function indexNadFun(
  env: { CIEL_STATE: KVNamespace; DB: D1Database; MARKET_DATA: R2Bucket; NAD_RPC_URL?: string },
  maxBlocks = 3000
  ...
  const createdTokens = new Map<string, { quoteToken: string; pair: string; symbol: string; name: string; block: bigint }>();
  const graduatedTokens = new Set<string>();
  const stats = new Map<string, { quoteVolume: bigint; buyVolume: bigint; sellVolume: bigint; buys: number; sells: number; liquidityQuote: bigint; pair?: string; quoteToken?: string }>();
  const touch = (token: string) => {
    let s = stats.get(token);
    if (!s) {
      s = { quoteVolume: 0n, buyVolume: 0n, sellVolume: 0n, buys: 0, sells: 0, liquidityQuote: 0n };
      stats.set(token, s);
    }
    return s;
  };
  ...
  for (const log of creates) {
    ...
    createdTokens.set(a.token.toLowerCase(), { quoteToken: a.quoteToken, pair: a.pair, symbol: a.symbol, name: a.name, block: log.blockNumber });
  ...
  for (const log of buys) {
    const a = log.args;
    if (!a.token) continue;
    const s = touch(a.token);
    s.quoteVolume += a.quoteIn ?? 0n;
    s.buyVolume += a.quoteIn ?? 0n;
    s.buys++;
  ...
  for (const log of sells) {
    ...
    if (!a.token) continue;
    const s = touch(a.token);
    s.quoteVolume += a.quoteOut ?? 0n;
    s.sellVolume += a.quoteOut ?? 0n;
    s.sells++;
  ...
  for (const log of graduates) {
    const a = log.args;
    if (!a.token) continue;
    graduatedTokens.add(a.token.toLowerCase());
  ...
  for (const [token, s] of stats) {
    ...
    const quoteOut = await quoteSell(client, tokenAddress, unit).catch(() => 0n);
    ...
    if (Number(meta.graduated) === 1 && meta.pair_address) liquidityUsd = await pairLiquidityUsd(client, meta.pair_address as Address, tokenAddress, monPrice);
    ...
    await env.DB.prepare(`INSERT INTO market_snapshots(token_address,ts_ms,price_usd,market_cap_usd,liquidity_usd,volume_5m_usd,buys_5m,sells_5m,holders,quote_token,buy_volume_usd,sell_volume_usd,source_block)
      VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(token, ts, priceUsd, marketCapUsd, liquidityUsd, volumeUsd, s.buys, s.sells, 0, meta.quote_token, buyVolumeUsd, sellVolumeUsd, Number(toBlock)).run();
  ...
  await env.CIEL_STATE.put("indexer_last_graduated_count", String(graduatedTokens.size));
