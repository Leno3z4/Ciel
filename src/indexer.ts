...
type MarketInfo = Record<string, unknown>;
type NadFunToken = { token_info?: Record<string, unknown>; market_info?: MarketInfo; percent?: number | string };
type ChartRow = { t: number; o: number; h: number; l: number; c: number; v: number };
type IndexerState = { nextBlock: string; latestBlock: string; lastSnapshotCount: number; lastRunMs?: number };

a const INDEXER_STATE_KEY = "indexer_state";
...
function address(v: string | null): v is `0x${string}` { return !!v && /^0x[a-fA-F0-9]{40}$/.test(v); }
...
function tokenAddress(item: NadFunToken): string | null { const a = str(item.token_info?.token_id); if (address(a)) return a; const b = str(item.token_info?.address); return address(b) ? b : null; }
...
function liquidity(item: NadFunToken): number { const direct = num(item.market_info?.liquidity_usd) || num(item.market_info?.liquidity); if (direct > 0) return direct; const r = num(item.market_info?.reserve_native_usd); if (r > 0) return r * 2; return 0; }
function volume5m(item: NadFunToken, fallback: number): number { const direct = num(item.market_info?.volume_5m_usd) || num(item.market_info?.volume5m_usd); if (direct > 0) return direct; return fallback; }
...
async function fetchRanking(env: IndexEnv): Promise<NadFunToken[]> {
  ...
  const url = `${API_BASE}/order/market_cap?page=1&limit=${MARKET_LIMIT}&is_nsfw=false`;
  try {
    const r = await fetch(url, { headers: { Accept: "application/json", "User-Agent": "Ciel-NadFun/2.0" }, cf: { cacheTtl: 600 } });
    if (r.ok) { const d = decode(await r.text()) as { tokens?: NadFunToken[] } | null; if (d && Array.isArray(d.tokens)) { await env.CIEL_STATE.put(RANKING_CACHE_KEY, JSON.stringify(d.tokens), { expirationTtl: 3600 }); await env.CIEL_STATE.put(RANKING_FETCH_TS_KEY, String(Date.now()), { expirationTtl: 3600 }); return d.tokens; } }
    else console.error(`NadFun market ranking HTTP ${r.status}`);
...
async function fetchChart(token: string, countback = 72): Promise<ChartRow[]> {
  const now = Math.floor(Date.now() / 1000);
  const from = now - 7 * 24 * 3600;
  const url = `${API_BASE}/trade/chart/${token}?resolution=60&from=${from}&to=${now}&countback=${countback}&chart_type=market_cap_usd`;
  try {
    const r = await fetch(url, { headers: { Accept: "application/json", "User-Agent": "Ciel-NadFun/2.0" }, cf: { cacheTtl: 300 } });
    if (!r.ok) return [];
    const d = await r.json() as { s?: string; t?: unknown[]; o?: unknown[]; h?: unknown[]; l?: unknown[]; c?: unknown[]; v?: unknown[] };
    if (d.s !== "ok" || !Array.isArray(d.t) || !Array.isArray(d.c)) return [];
    const rows: ChartRow[] = [];
    for (let i = 0; i < d.t.length; i++) { const c = num(d.c[i]); if (c > 0) rows.push({ t: num(d.t[i]), o: num(d.o?.[i]), h: num(d.h?.[i]), l: num(d.l?.[i]), c, v: num(d.v?.[i]) }); }
    return rows;
  } catch { return []; }
}

async function seedToken(env: IndexEnv, token: string, item?: NadFunToken): Promise<void> {
  const existing = await env.DB.prepare("SELECT address FROM tokens WHERE address=?").bind(token).first<{ address: string }>();
  if (existing) return;
  const decimals = Math.max(0, Math.floor(num(item?.market_info?.decimals ?? item?.token_info?.decimals) || 18));
  const supply = item?.market_info?.total_supply ?? item?.token_info?.total_supply ?? null;
  await env.DB.prepare(`INSERT INTO tokens(address,symbol,name,market_cap_usd,liquidity_usd,first_seen_ms,last_seen_ms,total_supply,decimals,quote_token,pair_address,graduated,created_at_block) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,NULL)`).bind(token, str(item?.token_info?.symbol), str(item?.token_info?.name), 0, 0, Date.now(), Date.now(), supply, decimals, quote(item || {}), pair(item || {}), graduated(item || {})).run();
}

async function bootstrapFactory(env: IndexEnv, client: ReturnType<typeof publicClient>): Promise<void> {
  const count = num((await env.DB.prepare("SELECT COUNT(*) as count FROM tokens").first<{ count: number }>())?.count);
  if (count > 0) return;
  try {
    ...
    for (let i = Math.max(0, total - 8); i < total; i++) {
      ...
      const [a, b] = await Promise.all([
        client.readContract({ address: p, abi: factoryPairAbi, functionName: "token0" }),
        client.readContract({ address: p, abi: factoryPairAbi, functionName: "token1" })
      ]);
      const aQuote = a.toLowerCase() === WMON.toLowerCase() || a.toLowerCase() === LVMON.toLowerCase();
      const bQuote = b.toLowerCase() === WMON.toLowerCase() || b.toLowerCase() === LVMON.toLowerCase();
      if (!aQuote && !bQuote) continue;
      const token = aQuote ? b : a; const q = aQuote ? a : b;
      await env.DB.prepare(`INSERT INTO tokens(address,symbol,name,market_cap_usd,liquidity_usd,first_seen_ms,last_seen_ms,total_supply,decimals,quote_token,pair_address,graduated,created_at_block) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,NULL) ON CONFLICT(address) DO UPDATE SET quote_token=excluded.quote_token,pair_address=excluded.pair_address,graduated=1,last_seen_ms=excluded.last_seen_ms`).bind(token, null, null, 0, 0, Date.now(), Date.now(), null, 18, q, p, 1).run();
    }
...
export async function indexNadFun(env: IndexEnv, maxBlocks = RPC_LOG_RANGE_BLOCKS): Promise<IndexResult | null> {
  ...
  const rawState = await env.CIEL_STATE.get(INDEXER_STATE_KEY); const state = rawState ? JSON.parse(rawState) as IndexerState : null;
  ...
  for (const x of creates) { const a = x.args; if (a.token && a.quoteToken && a.pair) { const [s, d] = await Promise.all([client.readContract({ address: a.token, abi: tokenMetaAbi, functionName: "totalSupply" }).catch(() => null), client.readContract({ address: a.token, abi: tokenMetaAbi, functionName: "decimals" }).catch(() => 18)]); await env.DB.prepare(`INSERT INTO tokens(address,symbol,name,market_cap_usd,liquidity_usd,first_seen_ms,last_seen_ms,total_supply,decimals,quote_token,pair_address,graduated,created_at_block) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(address) DO UPDATE SET last_seen_ms=excluded.last_seen_ms,total_supply=COALESCE(excluded.total_supply,tokens.total_supply),decimals=excluded.decimals,quote_token=excluded.quote_token,pair_address=excluded.pair_address`).bind(a.token, a.symbol ?? null, a.name ?? null, 0, 0, Date.now(), Date.now(), s?.toString() ?? null, Number(d), a.quoteToken, a.pair, 0, Number(x.blockNumber)).run(); } }
  for (const x of pairs) { const a = x.args; if (!a.token0 || !a.token1 || !a.pair) continue; const aQuote = a.token0.toLowerCase() === WMON.toLowerCase() || a.token0.toLowerCase() === LVMON.toLowerCase(); const bQuote = a.token1.toLowerCase() === WMON.toLowerCase() || a.token1.toLowerCase() === LVMON.toLowerCase(); if (!aQuote && !bQuote) continue; await env.DB.prepare(`INSERT INTO tokens(address,symbol,name,market_cap_usd,liquidity_usd,first_seen_ms,last_seen_ms,total_supply,decimals,quote_token,pair_address,graduated,created_at_block) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,NULL) ON CONFLICT(address) DO UPDATE SET quote_token=excluded.quote_token,pair_address=excluded.pair_address,graduated=1,last_seen_ms=excluded.last_seen_ms`).bind(aQuote ? a.token1 : a.token0, null, null, 0, 0, Date.now(), Date.now(), null, 18, aQuote ? a.token0 : a.token1, a.pair, 1).run(); }
  ...
  for (const item of ranked) { const t = tokenAddress(item); if (t) await seedToken(env, t, item); }
  ...
  const candidates = new Set<string>(); for (const item of ranked.slice(0, MARKET_LIMIT)) { const t = tokenAddress(item); if (t) candidates.add(t); } for (const row of existing.results ?? []) candidates.add(row.address);
  ...
  for (const token of candidates) {
    ...
    let cap = item ? marketCap(item, decimals) : 0;
    let px = item ? price(item) : 0;
    let liq = item ? liquidity(item) : 0;
    let vol = item ? volume5m(item, 0) : 0;
    let hold = item ? holders(item) : 0;
    let chart: ChartRow[] = [];
    if (!(cap >= MIN_MARKET_CAP_USD) || !(px > 0)) chart = await fetchChart(token, 72);
    if (chart.length) { const last = chart[chart.length - 1]; cap = cap >= MIN_MARKET_CAP_USD ? cap : last.c; if (!(px > 0)) { const supply = units(item?.market_info?.total_supply ?? item?.token_info?.total_supply, decimals) || (meta?.total_supply ? Number(BigInt(meta.total_supply)) / 10 ** decimals : 0); px = supply > 0 ? cap / supply : 0; } }
    if (!(cap >= MIN_MARKET_CAP_USD) || !(px > 0)) continue;
    const fallbackVol = chart.length ? chart[chart.length - 1].v : 0;
    vol = vol || fallbackVol;
    liq = liq || Number(meta?.liquidity_usd || 0);
    hold = hold || 0;
    const q = quote(item || {}) || meta?.quote_token || "NADFUN";
    const p = pair(item || {}) || meta?.pair_address || null;
    await env.DB.prepare(`INSERT INTO market_snapshots(token_address,ts_ms,price_usd,market_cap_usd,liquidity_usd,volume_5m_usd,buys_5m,sells_5m,holders,quote_token,buy_volume_usd,sell_volume_usd,source_block) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(token, now, px, cap, liq, vol, 0, 0, hold, q, 0, 0, Number(toBlock)).run();
    await env.DB.prepare("UPDATE tokens SET market_cap_usd=?,liquidity_usd=?,last_seen_ms=?,quote_token=COALESCE(?,quote_token),pair_address=COALESCE(?,pair_address),graduated=? WHERE address=?").bind(cap, liq, now, q, p, item ? graduated(item) : Number(meta?.graduated || 0), token).run();
    snapshots++;
  }
