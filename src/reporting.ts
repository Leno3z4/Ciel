...
function label(row: { symbol?: string | null; name?: string | null; address: string }): string {
  return row.symbol || row.name || shortAddress(row.address);
}
...
export async function reportAfterNotification(env: Env, sourceText: string, send: SendTelegram): Promise<void> {
  ...
  try {
    ...
    // Reporting must never break the underlying market/indexer/model notification.
...
async function sendMarketPulse(env: Env, send: SendTelegram): Promise<void> {
  ...
  const gainers = [...top]
    .map(row => ({ row, change: Number(row.previous_price_usd) > 0 ? ((Number(row.price_usd) - Number(row.previous_price_usd)) / Number(row.previous_price_usd)) * 100 : 0 }))
...
async function sendDiscoveryAlerts(env: Env, send: SendTelegram): Promise<string> {
  ...
  const now = Date.now();
  const rows = await env.DB.prepare(`
    SELECT address,symbol,name,first_seen_ms,graduated,market_cap_usd,liquidity_usd
    FROM tokens WHERE first_seen_ms > ? AND first_seen_ms <= ?
    ORDER BY first_seen_ms ASC LIMIT 12
  `).bind(cursor, now).all<DiscoveryRow>();
...
async function sendHourlyIntelligenceIfDue(env: Env, send: SendTelegram): Promise<void> {
  ...
  const signals = await env.DB.prepare(`
    SELECT s.id,s.token_address,s.action,s.confidence,s.anomaly_score,s.model,s.rationale,t.symbol,t.name
    FROM signals s LEFT JOIN tokens t ON t.address=s.token_address
    ORDER BY s.ts_ms DESC LIMIT 12
  `).all<{ id: number; token_address: string; action: string; confidence: number; anomaly_score: number; model: string; rationale: string; symbol: string | null; name: string | null }>();
  ...
  const positions = await env.DB.prepare(`
    SELECT p.token_address,p.quantity,p.entry_price_usd,p.last_price_usd,t.symbol,t.name,t.decimals
    FROM positions p LEFT JOIN tokens t ON t.address=p.token_address
    WHERE p.quantity <> '0' ORDER BY p.updated_ts_ms DESC LIMIT 8
  `).all<{ token_address: string; quantity: string; entry_price_usd: number; last_price_usd: number; symbol: string | null; name: string | null; decimals: number }>();
  ...
  const signalLines = (signals.results ?? []).slice(0, 6).map(s => `• ${s.action.padEnd(6)} ${label(s)} | ${(Number(s.confidence || 0) * 100).toFixed(0)}% | A ${Number(s.anomaly_score || 0).toFixed(2)}`);
